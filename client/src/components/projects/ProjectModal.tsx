...
import { adminGetCustomers } from '../../services/admin.service';
...
export const ProjectModal = ({ selected, formData, onChange, onSubmit, onClose }: Props) => {
  const { t } = useTranslation();
  const [customers, setCustomers] = useState<CustomerData[]>([]);
  const [tab, setTab] = useState<Tab>('details');
  const set = (patch: Partial<ProjectData>) => onChange({ ...formData, ...patch });

  useEffect(() => {
    adminGetCustomers().then(setCustomers).catch(console.error);
  }, []);
      ...
      <div className="bg-white rounded-xl shadow-xl border border-gray-100 w-full max-w-lg overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-gray-50 bg-gray-50/50">
          <h3 className="font-semibold text-gray-900">
            {selected ? t('admin.modal_title_edit', 'Edit Project') : t('admin.modal_title_create', 'Create Project')}
          </h3>
        </div>

        {/* Tabs — doar dacă edităm un proiect existent */}
        {selected && (
          <div className="flex border-b border-gray-100 px-6">
            {(['details', 'members'] as Tab[]).map(t_ => (
              <button
                key={t_}
                onClick={() => setTab(t_)}
        ...
        {tab === 'details' && (
          <form onSubmit={onSubmit} className="p-6 space-y-4 max-h-[65vh] overflow-y-auto">
            ...
            <Field label={t('admin.label_name', 'Project Name')}>
              <input
                required type="text"
                value={formData.name ?? ''}
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label={t('admin.label_status', 'Status')}>
                ...
                >
                  {STATUSES.map(s => (
                    <option key={s} value={s}>{t(`project_status.${s}`, s)}</option>
                  ))}
                </select>
              </Field>

              <Field label={t('admin.label_budget', 'Budget')}>
                <input
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label={t('admin.label_start', 'Start Date')}>
                <input
                  type="date"
        ...
        {/* Tab: Members — doar dacă proiectul există */}
        {tab === 'members' && selected?.id && (
          <div className="p-6 max-h-[65vh] overflow-y-auto">
            <ProjectMembersPanel projectId={selected.id} />
